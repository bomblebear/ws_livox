...
void mouse_callback(int event, int x, int y, int flags, void* param) {
    ...
    if (event == cv::EVENT_LBUTTONDOWN) {
      //cout << "X: " <<x <<"Y: " <<y << endl;
        ...
        cv::circle(src_img, cv::Point(x, y), 2,cv::Scalar(0, 0, 255),2);
        cv::putText(src_img, buff, cv::Point(x, y), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 0, 255), 1);
...
int main(int argc, char **argv) {
    ...
    //cv::setMouseCallback("source",NULL,NULL);
